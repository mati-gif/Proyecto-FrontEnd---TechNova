...
function ContactUs({ onSuccess }) {
    ...
    const handleForm = (event) => {
        ...
        if (name === "") {
            ...
            setErrors(prev => ({ ...prev, name: true, nameErrorDescription: "El nombre no puede estar vacío" }));
            return;
        }

        if (email === "") {
            emailInputRef.current.focus();
            setErrors(prev => ({ ...prev, email: true, emailErrorDescription: "El email es requerido" }));
            return;
        }

        if (!validateEmail(email)) {
            emailInputRef.current.focus();
            setErrors(prev => ({ ...prev, email: true, emailErrorDescription: "Formato de email inválido" }));
            return;
        }

        if (message === "") {
            messageInputRef.current.focus();
            setErrors(prev => ({ ...prev, message: true, messageErrorDescription: "El mensaje es requerido" }));
            return;
        }
    ...
    return (
        <Container className="py-5">
            <Row className="justify-content-center">
                <Col md={8} lg={6}>
                    <Card className="border-0 shadow mt-2 p-3" style={{ borderRadius: "16px" }}>
                        <Card.Body>
                            <div className="mb-4">
                                <h2 className="fw-bold text-dark mb-1">Formulario de Contacto</h2>
                                <p className="text-secondary small">Envianos tu consulta y nos pondremos en contacto a la brevedad.</p>
                            </div>
                            ...
                            <Form onSubmit={handleForm}>
                                ...
                                <Form.Group className="mb-3">
                                    <Form.Label className="d-flex align-items-center gap-2 fw-semibold small">
                                        <User size={16} className="text-primary" /> NOMBRE Y APELLIDO
                                    </Form.Label>
                                    <Form.Control
                                        ref={nameInputRef}
                                        value={name}
                                ...
                                <Form.Group className="mb-4">
                                    <Form.Label className="d-flex align-items-center gap-2 fw-semibold small">
                                        <MessageSquare size={16} className="text-primary" /> TU MENSAJE
                                    </Form.Label>
                                    <Form.Control
                                        ref={messageInputRef}
                                        as="textarea"
