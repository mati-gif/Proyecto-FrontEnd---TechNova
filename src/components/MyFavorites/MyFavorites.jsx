...
function MyFavorites() {
  ...
  return (
    <Container className="py-4 py-lg-5">
      ...
      <p className="text-secondary mb-4">Tus productos guardados y deseados en TechNova</p>

      {/* CASO VACÍO */}
      {(!favorites || favorites.length === 0) ? (
        <Card className="border text-center p-5 mx-auto" style={{ maxWidth: 600 }}>
          ...
          <p className="text-secondary mb-3">Guardá los productos que te interesen para verlos acá más tarde.</p>
          <div>
            <Button as={Link} to="/catalog" variant="primary">Explorar productos</Button>
          </div>
        </Card>
      ) : (
        ...
        <Row xs={2} sm={2} md={3} lg={4} xl={5} className="g-3">
          {favorites.map((product) => {
            ...
            return (
              <Col key={product.id}>
                <Card className="h-100 shadow-sm border position-relative d-flex flex-column justify-content-between tn-fav-card" style={{ borderRadius: "8px", overflow: "hidden" }}>
                  ...
                  >
                    ...
                    <Card.Body className="p-2 d-flex flex-column justify-content-between flex-grow-1">
                      <div>
                        {product.brand && (
                          <div className="text-uppercase text-muted fw-semibold mb-1" style={{ fontSize: "10px" }}>
                            {product.brand}
                          </div>
                        )}
                        <Card.Title 
                  ...
                  <div className="p-2 pt-0">
                    {quantity > 0 ? (
                      ...
                      <div className="d-flex align-items-center justify-content-between border rounded bg-light" style={{ height: "31px" }}>
                        {/* Botón Restar */}
                        <Button
                          variant="light"
                          size="sm"
                        ...
                        <span className="fw-bold text-dark small">{quantity}</span>
