...
function AdminUsers() {
  ...
  const handleRoleChange = async (userToUpdate, newRoleName) => {
    try {
      const response = await fetch(
        `http://localhost:3000/change/role/${userToUpdate.id}`,
        {
          method: "PUT",
          headers: {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/delete/user/${userToDelete.id}`,
        {
          method: "PUT",
          headers: {
            "Content-type": "application/json",
  ...
  const handleChangePassword = async (event) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/change/password/${userToChangePassword.id}`,
        {
          method: "PUT",
          headers: {
            "Content-type": "application/json",
                ...
                <tr key={u.id}>
                  <td>
                    <div className="d-flex align-items-center gap-2">
                      <span
                        className="rounded-circle bg-gradient-primary text-white d-inline-flex align-items-center justify-content-center fw-semibold"
                        style={{
                          width: 32,
                          height: 32,
                          fontSize: 13,
                          background:
                            "linear-gradient(135deg, #2b56f5, #5b8bff)",
                        }}
                      >
                  ...
                  <td>
                    <div className="d-flex align-items-center gap-2">
                      <Badge
                        bg={roleVariant(u.role.name.toLowerCase())}
