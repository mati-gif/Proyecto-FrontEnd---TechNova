...
function CheckOut() {
  ...
  const { shippingThreshold, shippingCost, finalTotal, cart, totalPrice } =
  ...
  const handleOpenModal = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || "Error al obtener las direcciones",
        );
      }
  ...
  const handleDesactivate = async (addressId) => {
    try {
      ...
      if (!address) return;
      setActiveAddresses((prev) => prev.filter((a) => a.id !== addressId));

      setInactiveAddresses((prev) => [...prev, { ...address, active: false }]);
  ...
  const handleActivate = async (addressId) => {
    ...
    try {
      ...
      setInactiveAddresses((prev) => prev.filter((a) => a.id !== addressId));

      setActiveAddresses((prev) => [...prev, { ...address, active: true }]);
  ...
  const filteredInactiveAddresses = inactiveAddresses.filter(
    (addr) =>
  ...
  const validateErrors = () => {
    ...
    if (form.zipCode.trim() === "") {
      newErrors.zipCode = "El código postal no puede estar vacío";
    }
  ...
  return (
    <Container className="py-4 py-lg-5">
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <p className="text-secondary mb-4"> Paso 1 de 2 </p>
                    ...
                    {cart.map((product) => (
                      <div key={product.id} className="d-flex gap-3">
                        <div className="position-relative flex-shrink-0">
                          ...
                          />
                          <span
                            className="position-absolute rounded-circle bg-dark text-white d-flex align-items-center justify-content-center"
                            style={{
                              top: -6,
                              right: -6,
                              width: 20,
                  ...
                  <div className="small">
                    ...
                    {shippingCost > 0 && (
                      <p
                        ...
                        style={{ fontSize: 12 }}
                      >
                        Te faltan {formatPrice(shippingThreshold - totalPrice)}{" "}
                        para tener el envío gratis
                      </p>
                    )}
                  </div>
                  <hr />
