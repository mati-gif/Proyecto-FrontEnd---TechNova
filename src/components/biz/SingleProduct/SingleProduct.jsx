...
function SingleProduct() {
  ...
  const discount = producto.originalPrice
    ? Math.round(
        ((producto.originalPrice - producto.price) / producto.originalPrice) *
          100,
      )
    : 0;
  ...
  return (
    <Container className="py-5">
      <Row className="align-items-center g-5">
        ...
        <Col md={6}>
          <div
            className="shadow-sm rounded overflow-hidden bg-light p-3 d-flex justify-content-center align-items-center position-relative"
          ...
          >
            ...
            <div className="position-absolute top-0 start-0 p-3 d-flex flex-column gap-2">
              ...
              {discount > 0 && (
                <Badge bg="danger" className="fs-6 px-3 py-2 shadow-sm">
                  -{discount}%
                </Badge>
              )}
            </div>
            ...
            {(isCartAnimating || isFavAnimating) && (
              <div className="tn-action-overlay">
                {isCartAnimating && (
                  <ShoppingCart
                    className="action-icon cart-pop action-color"
                    size={44}
                  />
                )}
                {isFavAnimating && (
                  <Heart
                    className="action-icon heart-pop action-color fav-color"
                    size={52}
                  />
                )}
              </div>
        ...
        <Col md={6}>
          <div className="ps-md-4">
            ...
            <h1 className="display-5 fw-bold text-dark mt-1 mb-3">
              {producto.name}
            </h1>
            ...
            <div className="single-product-actions d-flex align-items-center gap-3 mt-5">
              ...
              >
                <ShoppingCart
                  className={isCartAnimating ? "cart-pop" : ""}
                  size={20}
                />
